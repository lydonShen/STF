...
from fcos_core.modeling.rpn.fcos.fcos import FCOSModule
...
class ViewScaleBrightnessSearchEnv(gym.Env):
    ...
    def _get_loss(self):
        return FCOSModule(self.cfg, 256)
    ...
    def calculate_state1(self, features):
        state = []
        for f in features:
            af = self.get_aggregated_feature(f)
            # c h w --- h w c
    ...
    def calculate_benefit_loss(self,locations, box_cls, box_regression, centerness, targets):

        a=[]
        a.append(targets)
        targets=a
        losses = self.loss.cal_loss(locations, box_cls, box_regression, centerness, targets)
        return float(losses[1]["loss_cls"]) + float(losses[1]["loss_reg"]) +float(losses[1]["loss_centerness"])
    ...
    def calculate_feature_state(self, features):
    ...
    def reset(self):
        ...
        self.benefit = self.calculate_benefit_loss(locations, box_cls, box_regression, centerness,init_gt)
    ...
    def step(self, action):
        ...
        if not done:
            ...
            self.state = self.calculate_state(features)
            current_benefit = self.calculate_benefit_loss(locations, box_cls, box_regression, centerness, current_gt)
    ...
    def render(self):
        self.image_id=0
        init_img = np.array(self.dataset[self.image_id][0])
        imagename=self.dataset.get_img_info(self.image_id)['file_name'].split('.')[0]
        self.get_init_position()
        features, locations, box_cls, box_regression, centerness, image_sizes = self.detector.compute_state(init_img)
        state = self.calculate_state1(features)
        self.image_id = (self.image_id + 1) % self.dataset_length
        return None

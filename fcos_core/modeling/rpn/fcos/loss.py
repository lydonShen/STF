...
class FCOSLossComputation(object):
    ...
    def get_sample_region(self, gt, strides, num_points_per, gt_xs, gt_ys, radius=1.0):
        ...
        left = gt_xs.cuda()[:, None] - center_gt.cuda()[..., 0]
        right = center_gt.cuda()[..., 2] - gt_xs.cuda()[:, None]
        top = gt_ys.cuda()[:, None] - center_gt.cuda()[..., 1]
        bottom = center_gt.cuda()[..., 3] - gt_ys.cuda()[:, None]
    ...
    def compute_targets_for_locations(self, locations, targets, object_sizes_of_interest):
        ...
        for im_i in range(len(targets)):
            ...
            #print("targets_per_im.mode",targets_per_im.mode)
            assert targets_per_im.mode == "xyxy"
            bboxes = targets_per_im.bbox
            labels_per_im = targets_per_im.get_field("labels")
            area = targets_per_im.area()
            cuda = True if torch.cuda.is_available() else False
            l = xs.cuda()[:, None] - bboxes.cuda()[:, 0][None]
            t = ys.cuda()[:, None] - bboxes.cuda()[:, 1][None]
            r = bboxes.cuda()[:, 2][None] - xs.cuda()[:, None]
            b = bboxes.cuda()[:, 3][None] - ys.cuda()[:, None]
